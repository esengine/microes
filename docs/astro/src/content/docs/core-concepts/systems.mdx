...
1. **Parameters** — an array of `Query`, `Res`, `ResMut`, `Commands`, `EventWriter`, or `EventReader` descriptors
...
<Aside type="note">
  ...
  The maximum number of iterations is configurable via `RuntimeConfig`:

  ```typescript
  import { RuntimeConfig } from 'esengine';
  RuntimeConfig.maxFixedSteps = 8; // default
  ```
</Aside>
...
```typescript
defineSystem(
    [Commands(), Res(Time), Res(Input), Query(Mut(Transform), Velocity)],
