...
2D sprite rendering. Requires `Transform` to be visible. See [Sprite guide](/microes/guides/sprite/) for full details.
...
Spine skeletal animation playback. Requires `Transform`. See [Spine Animation guide](/microes/guides/spine/) for full details.

### BitmapText

GPU-rendered text using pre-baked bitmap font atlases (BMFont format). Requires `Transform`. See [Bitmap Text guide](/microes/guides/bitmap-text/) for full details.
...
UI text rendering. Works with `UIRect` and `Sprite` to display text. See [Text & Image](/microes/guides/ui-text/) for full details.
...
Clips child entities to the parent's `UIRect` bounds. Requires `UIRect` and `Transform` on the same entity. See [Masking & SafeArea](/microes/guides/ui-masking/) for details.
