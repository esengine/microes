---
title: Components
description: Builtin and custom components in Estella
---
...
<Aside type="note">
  In previous versions, `LocalTransform` and `WorldTransform` were separate components. They have been unified into a single `Transform`. The old names still work as aliases but `Transform` is the recommended import.
</Aside>

### Sprite

2D sprite rendering. Requires `Transform` to be visible. See [Sprite guide](/docs/guides/sprite/) for full details.
...
Spine skeletal animation playback. Requires `Transform`. See [Spine Animation guide](/docs/guides/spine/) for full details.

### BitmapText

GPU-rendered text using pre-baked bitmap font atlases (BMFont format). Requires `Transform`. See [Bitmap Text guide](/docs/guides/bitmap-text/) for full details.
...
UI text rendering. Works with `UIRect` and `Sprite` to display text. See [Text & Image](/docs/guides/ui-text/) for full details.
...
Clips child entities to the parent's `UIRect` bounds. Requires `UIRect` and `Transform` on the same entity. See [Masking & SafeArea](/docs/guides/ui-masking/) for details.
...
Physics components require the `PhysicsPlugin`. See [Physics guide](/docs/guides/physics/) for setup and usage.
...
- [Systems](/docs/core-concepts/systems/) — logic that operates on components
- [Queries](/docs/core-concepts/queries/) — finding entities by component
