...
GPU-rendered text using pre-baked bitmap font atlases (BMFont format). Requires `LocalTransform`. See [Bitmap Text guide](/microes/guides/bitmap-text/) for full details.
...
Clips child entities to the parent's `UIRect` bounds. Requires `UIRect` and `LocalTransform` on the same entity. See [UI & Text guide](/microes/guides/ui/) for details.
...
Tag component that marks an entity as a screen-space UI root. Children of a ScreenSpace entity are laid out in screen coordinates via their `UIRect`. Requires `UIRect` and `LocalTransform`.
