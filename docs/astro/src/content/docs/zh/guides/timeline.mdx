...
| AnimFrames | 可视化帧轨道——每个精灵帧显示为带颜色、可调整大小的色块，拥有独立时长 |
| Audio | 在特定时间触发音频事件 |
| Activation | 在时间范围内启用/禁用实体 |
| Marker | 在特定时间放置命名标记 |
| Custom Event | 在特定时间触发自定义事件 |
...
进入 Play 模式时，时间轴自动切换到 **LIVE** 模式。播放头实时跟踪运行时动画位置，工具栏显示红色"LIVE"指示器。LIVE 模式下编辑器播放控件被禁用，退出 Play 模式后恢复。
