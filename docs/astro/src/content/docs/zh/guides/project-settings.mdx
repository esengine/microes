---
title: 项目设置
description: 配置 Estella 项目的运行时、构建和编辑器参数
---
...
使用顶部的**搜索栏**按名称、描述或标签筛选设置。有非默认值的设置会在导航栏显示**圆点标记**。每个分类提供**重置**按钮，可将所有设置恢复为默认值。
...
可定义最多 16 个命名碰撞层（如 "Player"、"Enemy"、"Ground"），通过碰撞矩阵控制哪些层之间可以交互。详见[碰撞体 — 碰撞过滤](/docs/zh/guides/physics/colliders/#碰撞过滤)。
