...
对于 Playable Ad 构建，可以使用 `registerEmbeddedAssets` 将资源直接嵌入输出包中，避免运行时网络请求：
...
第一个参数是 `App` 实例，资源值必须是 data URI。注册后，所有 `assets.loadTexture()` / `assets.loadJson()` 调用将从嵌入数据解析，不再发起网络请求。

<Aside type="tip">
  编辑器的 Playable 构建流程会自动调用 `registerEmbeddedAssets`。仅在自定义构建流程中才需要手动调用。
</Aside>
