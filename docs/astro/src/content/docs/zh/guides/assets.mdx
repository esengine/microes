---
title: 资源加载
description: 在 Estella 中加载纹理、Spine 数据和其他资源
---
...
<Aside type="note">
  纹理使用引用计数。每次 `loadTexture()` 调用递增计数；`releaseTexture()` 递减。GPU 资源仅在计数归零时释放。
</Aside>
...
详见[预制体](/docs/zh/guides/prefabs/)了解完整的预制体 API。
...
详见[材质与着色器](/docs/zh/guides/materials/)了解完整的材质 API 和 `.esmaterial` 文件格式。
...
- [材质与着色器](/docs/zh/guides/materials/) — 自定义着色器和混合模式
- [精灵](/docs/zh/guides/sprite/) — 纹理与精灵的使用
- [位图文本](/docs/zh/guides/bitmap-text/) — 使用位图字体渲染文本
- [Spine 动画](/docs/zh/guides/spine/) — Spine 骨骼动画
