---
title: 渲染
description: Estella 中的相机、变换和渲染顺序
---

import { Aside } from '@astrojs/starlight/components';

Estella 使用基于组件的渲染系统。在场景编辑器中为实体添加渲染组件，C++ 后端自动处理绘制。

各渲染组件的详细指南参见[精灵](/docs/zh/guides/sprite/)和 [Spine 动画](/docs/zh/guides/spine/)。
...
2D 游戏最佳选择。`orthoSize` 决定可见的世界范围（默认：`540`）。在检查器中将 `projectionType` 设为 `ProjectionType.Orthographic`。从 `'esengine'` 导入 `ProjectionType` 和 `ClearFlags`。

### Canvas 集成

要进行屏幕适配和分辨率缩放，在相机实体上附加 `Canvas` 组件。详见 [Canvas 与分辨率](/docs/zh/guides/canvas/)。
...
- 小地图相机：视口 `(0.75, 0.75, 0.25, 0.25)`，`priority: 1`，`clearFlags: ClearFlags.ColorAndDepth`
...
每个可见实体都需要 `Transform`。在场景编辑器中添加并在检查器中配置位置、旋转和缩放。
...
自定义绘制回调在每个相机 pass 中的内置精灵、文本和 Spine 渲染之后执行。使用 `Renderer.setStage()` 在渲染管线中控制当前活跃的阶段：
...
- [Canvas 与分辨率](/docs/zh/guides/canvas/) — 屏幕适配与缩放模式
- [精灵](/docs/zh/guides/sprite/) — 精灵渲染和动画
- [Spine 动画](/docs/zh/guides/spine/) — 骨骼动画
- [位图文本](/docs/zh/guides/bitmap-text/) — GPU 渲染的位图字体文本
- [自定义绘制](/docs/zh/guides/custom-draw/) — 即时模式绘图，用于调试可视化和动态图形
- [后处理效果](/docs/zh/guides/post-processing/) — 全屏效果（模糊、暗角、灰度）
- [渲染纹理](/docs/zh/guides/render-texture/) — 离屏渲染，用于小地图、镜像和特效
- [几何体与网格](/docs/zh/guides/geometry/) — 自定义网格创建
