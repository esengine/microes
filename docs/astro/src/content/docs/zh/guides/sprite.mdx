---
title: 精灵
description: Estella 中的 2D 精灵渲染
---

import { Aside } from '@astrojs/starlight/components';

`Sprite` 组件是显示 2D 图形的主要方式。在场景编辑器中将它和 `Transform` 一起添加到实体上即可显示。
...
| `size` | Vec2 | `{x: 32, y: 32}` | 宽高（像素） |
| `uvOffset` | Vec2 | `{x: 0, y: 0}` | 纹理坐标偏移 |
| `uvScale` | Vec2 | `{x: 1, y: 1}` | 纹理坐标缩放 |
| `layer` | number | `0` | 渲染顺序（越大越靠前） |
| `flipX` | boolean | `false` | 水平翻转 |
| `flipY` | boolean | `false` | 垂直翻转 |
| `material` | number | `0` | 材质 ID |
| `enabled` | boolean | `true` | 是否渲染精灵 |

## 基本用法

在场景编辑器中：创建实体 → 添加 `Transform` 和 `Sprite` → 在检查器中设置尺寸和颜色。
...
完整纹理 API（`getTexture`、`releaseTexture`、九宫格元数据等）详见[资源加载](/docs/zh/guides/assets/)。
