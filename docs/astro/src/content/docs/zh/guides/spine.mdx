...
`SpineAnimation` 组件播放 [Spine](http://esotericsoftware.com/) 骨骼动画。在场景编辑器中将它和 `Transform` 一起添加到实体上。
...
2. 在场景编辑器中：创建实体 → 添加 `Transform` 和 `SpineAnimation`
...
ESEngine 同时支持 Spine **3.8**、**4.1** 和 **4.2** 版本。引擎会自动从骨骼数据中检测 Spine 版本并选择对应的运行时后端。
