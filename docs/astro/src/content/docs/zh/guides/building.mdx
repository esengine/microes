---
title: 构建
description: 将 Estella 项目构建到不同平台
---

import { Aside } from '@astrojs/starlight/components';

Estella 项目可以从编辑器构建到两个目标平台：**可玩广告**（单个自包含 HTML 文件）和**微信小游戏**（可直接在微信开发者工具中打开的目录结构）。
...
首次构建前，编辑器需要 C++ 工具链来编译引擎 WASM 二进制。打开**构建设置**，展开 **Toolchain (emsdk)** 部分。状态徽章显示工具链是否就绪：
...
<Aside type="note">
工具链只需安装一次，所有项目共享使用。它存储在编辑器的应用数据目录中，跨版本更新保留。
</Aside>
