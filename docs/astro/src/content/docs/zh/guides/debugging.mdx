...
**解决**:
- 访问前检查实体是否存在：`if (world.valid(entity)) { ... }`
- 避免跨帧存储实体 ID 而不进行验证
...
- [自定义绘制](/docs/zh/guides/custom-draw/) -- 使用 Draw API 进行调试可视化
- [构建与导出](/docs/zh/guides/building/) -- 构建流水线和配置
- [场景](/docs/zh/guides/scenes/) -- 场景加载和管理
