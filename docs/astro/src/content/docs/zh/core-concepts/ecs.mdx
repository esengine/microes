---
title: ECS 架构
description: 理解 Estella 中的实体-组件-系统模式
---

import { Aside } from '@astrojs/starlight/components';

Estella 使用实体-组件-系统（ECS）架构管理游戏对象。ECS 将**身份**（Entity）、**数据**（Component）和**行为**（System）分离。

## 什么是 ECS？

| 概念 | 角色 | 示例 |
|------|------|------|
| **实体 (Entity)** | 唯一标识符（只是一个数字） | 玩家、敌人、子弹 |
| **组件 (Component)** | 附加在实体上的数据 | Transform、Sprite、Health |
| **系统 (System)** | 操作具有特定组件的实体的逻辑 | 移动系统、渲染系统 |

<Aside type="tip">
  ECS 倡导**组合优于继承**。不需要深层类继承结构，而是通过组合小型、可复用的组件来构建游戏对象。
</Aside>

## 为什么选择 ECS？

传统 OOP 方式：

```
GameObject
├── Player extends GameObject
│   ├── FlyingPlayer extends Player
│   └── SwimmingPlayer extends Player
└── Enemy extends GameObject
    └── FlyingEnemy extends Enemy
```

ECS 方式 — 自由组合行为：

```
Player  = Entity + Transform + Sprite + Health + PlayerInput
Enemy   = Entity + Transform + Sprite + Health + AI
Bullet  = Entity + Transform + Sprite + Velocity + Damage
```

优势：

- **组合** — 自由搭配组件，无需僵化的继承层级
- **数据局部性** — 同类型组件连续存储，提升缓存性能
- **关注点分离** — 系统包含逻辑，组件持有数据
- **灵活性** — 运行时添加或移除组件来改变行为

## 实体

实体是一个轻量级句柄，由两部分组成：**索引**和**世代**。索引标识内存中的槽位，世代追踪该槽位被回收利用的次数。

这种 **generational handle** 设计意味着，当实体被销毁且其槽位被新实体复用时，任何指向旧实体的过期引用都会被自动检测——它们的世代号不匹配。这防止了代码意外操作已回收实体的 bug。

实际使用中不需要关心世代——只需将 `Entity` 类型当作不透明句柄使用。实体通常在**场景编辑器**中创建，也可以通过 `Commands` 在运行时生成：

```typescript
import { defineSystem, addStartupSystem, Commands, type Entity } from 'esengine';

addStartupSystem(defineSystem([Commands()], (cmds) => {
    const bullet: Entity = cmds.spawn().id();
    cmds.despawn(bullet);
}));
```

## 组件

组件是附加在实体上的数据。在代码中**定义**组件，然后在场景编辑器中**挂载**到实体上。

- **内置组件** — Transform、Sprite、Camera 等（编辑器中始终可用）
- **自定义组件** — 用 `defineComponent` / `defineTag` 定义（保存后出现在编辑器中）

```typescript
import { defineComponent, defineTag } from 'esengine';

const Health = defineComponent('Health', { current: 100, max: 100 });
const Player = defineTag('Player');
```

定义后，在编辑器中选择实体 → 添加组件 → 从菜单中选择 `Health` 或 `Player`。

详见[组件](/docs/zh/core-concepts/components/)。

## 系统

系统是查询具有特定组件的实体并操作它们的函数。它们自动运行在场景中所有匹配的实体上。

```typescript
import { defineSystem, addSystem, Res, Time, Query, Mut, Transform } from 'esengine';
import { Health } from './components';

addSystem(defineSystem(
    [Res(Time), Query(Mut(Transform), Health)],
    (time, query) => {
        for (const [entity, transform, health] of query) {
            if (health.current <= 0) {
                transform.position.y -= 100 * time.delta;
            }
        }
    }
));
```

详见[系统](/docs/zh/core-concepts/systems/)。

## ECS 流程

```
 ┌──────────────────────┐
 │  编辑器：场景搭建     │  放置实体，挂载组件
 └──────────┬───────────┘
            ▼
 ┌──────────────────────┐
 │  脚本：定义           │  defineComponent + defineSystem + addSystem
 └──────────┬───────────┘
            ▼
 ┌──────────┐
 │  启动    │  启动系统运行一次
 └────┬─────┘
      ▼
 ┌──────────┐
 │  更新    │◄─┐  系统每帧查询场景中的实体
 └────┬─────┘  │
      ▼        │
 ┌──────────┐  │
 │  渲染    │  │  C++ 后端绘制所有 Sprite/Camera
 └────┬─────┘  │
      └────────┘
```

1. **场景搭建** — 在编辑器中放置实体并挂载组件
2. **脚本** — 定义自定义组件并注册系统
3. **启动** — 启动系统运行一次
4. **更新循环** — 系统每帧查询和处理场景中的实体
5. **渲染** — C++ 后端自动渲染所有拥有 `Sprite` 和 `Camera` 组件的实体

## 如何与 ECS 交互

`World` 存储所有实体和组件。在脚本中，通过系统参数与之交互：

| 你想做什么 | 系统参数 |
|-----------|---------|
| 遍历具有特定组件的实体 | `Query(ComponentA, ComponentB)` |
| 读取组件数据 | `Query(Component)` — 数据随迭代获取 |
| 修改组件数据 | `Query(Mut(Component))` — 用 `Mut` 包裹 |
| 创建新实体 | `Commands()` → `cmds.spawn()` |
| 销毁实体 | `Commands()` → `cmds.despawn(entity)` |
| 运行时添加组件 | `Commands()` → `cmds.entity(e).insert(Component, data)` |
| 运行时移除组件 | `Commands()` → `cmds.entity(e).remove(Component)` |
| 读取资源 | `Res(ResourceType)` |
| 修改资源 | `ResMut(ResourceType)` |
| 响应新增组件 | `Query(Added(Component))` |
| 响应变更组件 | `Query(Changed(Component))` |
| 响应移除组件 | `Removed(Component)` |
| 系统间发送事件 | `EventWriter(Event)` / `EventReader(Event)` |
| 设置父子层级 | `GetWorld()` → `world.setParent(child, parent)` |
| 移除父节点 | `GetWorld()` → `world.removeParent(entity)` |

详见[系统](/docs/zh/core-concepts/systems/)和[查询](/docs/zh/core-concepts/queries/)。

## 下一步

- [组件](/docs/zh/core-concepts/components/) — 内置和自定义组件
- [系统](/docs/zh/core-concepts/systems/) — 定义和调度系统
- [查询](/docs/zh/core-concepts/queries/) — 查询实体
- [资源](/docs/zh/core-concepts/resources/) — 全局单例数据
- [变更检测](/docs/zh/core-concepts/change-detection/) — 响应组件变更
- [事件](/docs/zh/core-concepts/events/) — 解耦的系统间通信
