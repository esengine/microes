...
<Aside type="note">
  Fixed 系列调度（`FixedPreUpdate`、`FixedUpdate`、`FixedPostUpdate`）以 1/60s 固定步长运行，使用累加器驱动。一帧内可能执行零次或多次，取决于帧时长。适用于物理等需要一致时间步的确定性逻辑。
</Aside>
...
<Aside type="note">
  Commands 是缓冲的，在当前调度阶段结束时才应用。例外是 `.id()` — 调用它会立即创建实体，使 ID 可以马上使用。
</Aside>
