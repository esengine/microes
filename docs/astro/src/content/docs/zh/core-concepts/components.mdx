---
title: 组件
description: Estella 中的内置和自定义组件
---

import { Aside } from '@astrojs/starlight/components';

组件是附加在实体上的数据。在 Estella 中，你在代码中**定义**组件，然后在场景编辑器中**挂载**到实体上。
...
| `worldPosition` | Vec3 | 世界坐标（只读） |
| `worldRotation` | Quat | 世界旋转（只读） |
| `worldScale` | Vec3 | 世界缩放（只读） |

<Aside type="note">
  在之前的版本中，`LocalTransform` 和 `WorldTransform` 是两个独立组件。现在已统一为单一的 `Transform`。旧名称仍可作为别名使用，但推荐导入 `Transform`。
</Aside>

### Sprite

2D 精灵渲染。需要 `Transform` 才能显示。详见[精灵指南](/docs/zh/guides/sprite/)。
...
Spine 骨骼动画播放。需要 `Transform`。详见 [Spine 动画指南](/docs/zh/guides/spine/)。

### BitmapText

使用预烘焙位图字体图集（BMFont 格式）进行 GPU 渲染的文本。需要 `Transform`。详见[位图文本指南](/docs/zh/guides/bitmap-text/)。
...
| `enabled` | boolean | 是否渲染该组件 |

### Text

UI 文本渲染。配合 `UIRect` 和 `Sprite` 组件显示文本。详见[文本与图片](/docs/zh/guides/ui-text/)。
...
| `wordWrap` | boolean | 是否启用自动换行 |
| `overflow` | TextOverflow | 溢出行为：`Visible` (0)、`Clip` (1)、`Ellipsis` (2) |
...
将子实体裁剪到父级 `UIRect` 边界内。需要在同一实体上同时挂载 `UIRect` 和 `Transform`。详见[遮罩与安全区](/docs/zh/guides/ui-masking/)。

| 属性 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `enabled` | boolean | `true` | 是否启用裁剪 |
| `mode` | number | `0` | 遮罩模式：0=Scissor，1=Stencil |
...
物理组件需要 `PhysicsPlugin`。详见[物理指南](/docs/zh/guides/physics/)。
...
这适用于内置组件（如 `Interactable`、`UIMask`、`RigidBody`），也适用于任何包含 `enabled` 字段的自定义组件：
...
- [系统](/docs/zh/core-concepts/systems/) — 操作组件的逻辑
- [查询](/docs/zh/core-concepts/queries/) — 按组件查找实体
