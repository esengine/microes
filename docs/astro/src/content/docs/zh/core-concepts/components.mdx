...
2D 精灵渲染。需要 `Transform` 才能显示。详见[精灵指南](/microes/zh/guides/sprite/)。
...
Spine 骨骼动画播放。需要 `Transform`。详见 [Spine 动画指南](/microes/zh/guides/spine/)。

### BitmapText

使用预烘焙位图字体图集（BMFont 格式）进行 GPU 渲染的文本。需要 `Transform`。详见[位图文本指南](/microes/zh/guides/bitmap-text/)。
...
UI 文本渲染。配合 `UIRect` 和 `Sprite` 组件显示文本。详见[文本与图片](/microes/zh/guides/ui-text/)。
...
将子实体裁剪到父级 `UIRect` 边界内。需要在同一实体上同时挂载 `UIRect` 和 `Transform`。详见[遮罩与安全区](/microes/zh/guides/ui-masking/)。
...
- [变更检测](/microes/zh/core-concepts/change-detection/) — 响应组件变更
