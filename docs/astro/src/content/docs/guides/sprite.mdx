---
title: Sprite
description: 2D sprite rendering in Estella
---

import { Aside } from '@astrojs/starlight/components';

The `Sprite` component is the primary way to display 2D graphics. Add it to an entity in the scene editor along with `Transform` to make the entity visible.
...
| `uvOffset` | Vec2 | `{x: 0, y: 0}` | Texture coordinate offset |
| `uvScale` | Vec2 | `{x: 1, y: 1}` | Texture coordinate scale |
| `layer` | number | `0` | Render order (higher = on top) |
| `flipX` | boolean | `false` | Flip horizontally |
| `flipY` | boolean | `false` | Flip vertically |
| `pivot` | Vec2 | `{x: 0.5, y: 0.5}` | Pivot point for rotation and scaling |
| `material` | number | `0` | Material ID |
| `enabled` | boolean | `true` | Whether this sprite is rendered |

## Basic Usage

In the scene editor: create an entity → add `Transform` and `Sprite` → set size and color in the inspector.
...
See [Asset Loading](/docs/guides/assets/) for the full texture API (`getTexture`, `releaseTexture`, 9-slice metadata, etc.).
...
```typescript
import { defineSystem, addSystem, Commands, Sprite, Transform } from 'esengine';
