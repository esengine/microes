...
Scenes are managed in the editor — create `.scene` files and configure them in the project settings. For advanced use cases, you can register scenes programmatically in a `setup` function:
...
export function setup(app: App) {
    ...
    mgr.register({ name: 'game', path: '/scenes/game.json' });
    mgr.setInitial('menu');
}
```
