...
Double-click a `.esprefab` file in the content browser to enter **prefab edit mode**. The editor loads the prefab as a temporary scene where you can modify entities directly. Changes are saved back to the `.esprefab` file when you exit.
...
Prefabs can contain references to other prefabs. When instantiating, nested prefabs are automatically loaded and flattened. This requires an `assetServer` in the options:
