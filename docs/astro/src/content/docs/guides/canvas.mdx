...
description: Screen adaptation with Canvas component in Estella
---
...
| `scaleMode` | ScaleMode | `FixedHeight` (1) | Scale strategy (see below). Import `ScaleMode` from `'esengine'` |
...
```typescript
import { defineSystem, addSystem, Query, Mut, Canvas, ScaleMode } from 'esengine';

addSystem(defineSystem(
    [Query(Mut(Canvas))],
    (query) => {
        for (const [entity, canvas] of query) {
            canvas.scaleMode = ScaleMode.Match;
