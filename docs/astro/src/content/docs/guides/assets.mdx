...
registerEmbeddedAssets({
    'assets/player.png': 'data:image/png;base64,iVBOR...',
    'assets/config.json': '{"level":1}',
});
```

Once registered, all `assets.loadTexture()` / `assets.loadJson()` calls resolve from the embedded data instead of fetching over the network.
