...
<Aside type="note">
  Since v0.3.0, Spine uses standalone WASM modules (`spine38` / `spine42`) instead of being bundled into the engine core. You must register a Spine plugin before using Spine components.
</Aside>

## Spine Plugin Setup

Initialize the Spine plugin at startup to load the appropriate runtime version:
...
`SpinePlugin` accepts `'3.8'` or `'4.2'` to match your Spine editor export version. The plugin downloads the corresponding WASM module on first use.
